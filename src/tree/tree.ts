...
export class Tree<T> implements Monad<T> {
    ...
    public pipe<U>(f: (a: T) => Tree<U>): Tree<U> {
        return Tree.pipe(() => this, f)(null)
    }
    ...
    public static pipe<S, U, V>(left: (s: S) => Tree<U>, right: (u: U) => Tree<V>): (s: S) => Tree<V> {
        return (s) => Tree.flatMap(left(s), right)
    }
