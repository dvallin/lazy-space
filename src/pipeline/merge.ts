...
export abstract class Merge<L, R, T> implements Push2<L, R>, Source<T> {
    ...
    protected readonly subscriptions: Push<T>[] = []
    ...
    private tryPush(): Eval<void> {
        ...
        return t.map(e => e.flatMap(o =>
            flattenEvals(this.sendToSubscribers(o)).orElse(() => Eval.noop())
        )).orElse(() => Eval.noop())
    }

    private sendToSubscribers(output: T): Eval<void>[] {
        return this.subscriptions.map(s => s.push(output))
    }
